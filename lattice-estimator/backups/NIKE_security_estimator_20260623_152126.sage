...
#
# Supports:
#   · Preset parameter sets (Frost-128/192/256/384/512)
...
#   name, level, n, m, q, p_pk, p_u, p_v, eta_s, eta_r, b_msg
CUSTOM_PARAMS = dict(
    name   = "Custom-256",
    level  = 256,
    n      = 1024,
...
PRESETS = OrderedDict([
    ("Frost-128", dict(
        level=128,  n=512,   m=512,
        q=2**15,    p_pk=2**10, p_u=2**10, p_v=2**5,
        eta_s=2,    eta_r=2,   b_msg=2,
    )),
    ("Frost-192", dict(
        level=192,  n=880,   m=880,
        q=2**16,    p_pk=2**11, p_u=2**11, p_v=2**6,
        eta_s=1,    eta_r=1,   b_msg=3,
    )),
    ("Frost-256", dict(
        level=256,  n=1288,  m=1288,
        q=2**16,    p_pk=2**13, p_u=2**12, p_v=2**8,
        eta_s=1,    eta_r=1,   b_msg=4,
    )),
    ("Frost-384", dict(
        level=384,  n=1928,  m=1928,
        q=2**16,    p_pk=2**13, p_u=2**13, p_v=2**9,
        eta_s=1,    eta_r=1,   b_msg=4,
    )),
    ("Frost-512", dict(
        level=512,  n=2600,  m=2600,
        q=2**16,    p_pk=2**14, p_u=2**14, p_v=2**7,
        eta_s=1,    eta_r=1,   b_msg=4,
    )),
])
...
def validate_params(P):
    ...
    for key in ["p_pk", "p_u", "p_v"]:
        ...
        if q % p != 0:
            raise ValueError("[%s] %s must divide q" % (name, key))

    if P["level"] % 8 != 0:
        raise ValueError("[%s] level must be divisible by 8" % name)
...
def estimate_surface(tag, lwe_n, q, m_samples, eta_secret,
    ...
    print("")
    print("  Surface: %s" % tag)
    print("    LWE dim n = %d,  samples m = %d,  q = %s" %
...
def main():
    print("=" * 72)
    print("  LWE Security Estimator  (power-of-two modulus KEMs)")
    print("=" * 72)
